...
class User(Base):
    ...
    following = relationship("Follow", foreign_keys="[Follow.userID]", back_populates="follower", cascade="all, delete-orphan")
    followers = relationship("Follow", foreign_keys="[Follow.followID]", back_populates="followee", cascade="all, delete-orphan")
...
class Post(Base):
    ...
    update_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
...
class Comment(Base):
    ...
    parentcommentID = Column(Integer, ForeignKey("comment.commentID", ondelete="CASCADE"))
    create_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
...
class Follow(Base):
    ...
    follow_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
